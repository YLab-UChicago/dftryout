...
#Run with "python 34_ResNet.py <number of trials>"
...
def ResNet34(input_shape=(224, 224, 3), classes=1000):
    ...
    for _ in range(3): x = residual_block(x, 128)

    x = residual_block(x, 256, stride=2, conv_shortcut=True)
    for _ in range(5): x = residual_block(x, 256)
...
with open('quantized_resnet34.tflite', 'wb') as f:
    f.write(quantized_tflite_model)

# Load TFLite model and allocate tensors
interpreter = tf.lite.Interpreter(model_path='quantized_resnet34.tflite')
...
with open('result.txt', 'w') as file:
    ...
    total_time /= num_runs
    result_str = f"Average execution time for 8bit-quantized ResNet-34: {total_time:.4f} ms\n{num_runs} trials completed"
    print(result_str)
