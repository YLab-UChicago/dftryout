...
# Run with "python 13_VGG.py <number of trials>"
...
def VGG13(input_shape=(224, 224, 3), classes=1000):
...
with open('quantized_vgg13.tflite', 'wb') as f:
    f.write(quantized_tflite_model)

# Load TFLite model and allocate tensors
interpreter = tf.lite.Interpreter(model_path='quantized_vgg13.tflite')
...
with open('result.txt', 'w') as file:
    ...
    total_time /= num_runs
    result_str = f"Average execution time for 8bit-quantized VGG-13: {total_time:.4f} ms\n{num_runs} trials completed"
    print(result_str)
