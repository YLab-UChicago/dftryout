...
int main(int argc, char *argv[])
{
    ...
    */
    FILE *pFile = fopen("durations/vgg19_conv4_ws.txt", "a");
    int height;
    int width;
    int depth;
    int filter_height;
                ...
                for (int h = 0; h < out_height; h++)
                {
                    for (int w = 0; w < out_width; w++)
                    {
                        ...
                        if ((input_h < height && input_h >= 0) && (input_w < width && input_w >=0)) {
                            ...
                            outputs[h * out_width * num_filters + w * num_filters + f] += depth - 2 * (vaddvq_u8(vcntq_u8(output.val[0]))+vaddvq_u8(vcntq_u8(output.val[1]))+vaddvq_u8(vcntq_u8(output.val[2]))+vaddvq_u8(vcntq_u8(output.val[3])));
                        }
                    }
                }
            }
