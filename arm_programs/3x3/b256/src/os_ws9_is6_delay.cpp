    ...
    for (int f = 0; f < num_filters; f++)
    {
        ...
        for (int h = 0; h < out_height; h++)
        {
            for (int w = 0; w < out_width; w++)
            {
                ...
                output.val[0] = vaddq_u64(output.val[0],vcntq_u8(data1.val[0]));
                output.val[1] = vaddq_u64(output.val[1],vcntq_u8(data1.val[1]));
                ...
                outputs[h * out_width * num_filters + w * num_filters + f] = vaddvq_u8(output.val[0])+vaddvq_u8(output.val[0]);
