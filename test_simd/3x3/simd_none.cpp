...
{
    FILE *pFile = fopen("durations/simd_none.txt", "a");
    int height;
    int width;
    int depth;
    int filter_height;
                    ...
                    for (int i = 0; i < filter_height; i++)
                    {
                        for (int j = 0; j < filter_width; j++)
                        {
                            ...
                            if (input_h >= 0 && input_h < height && input_w >= 0 && input_w < width)
                            {
                                ...
                                outputs[h * out_width * num_filters + w * num_filters + f] += 256 - 2 * (data_to_process[0] + data_to_process[1] + data_to_process[2] + data_to_process[3]);
                            }
                        }
                    }
                }
