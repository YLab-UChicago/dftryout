...
{
    FILE *pFile = fopen("durations/simd_is_os5.txt", "a");
    int height;
    int width;
    int depth;
    int filter_height;
    ...
    for (int f = 0; f < num_filters; f++)
    {
        ...
        for (int h = 0; h < height; h++)
        {
            for (int w = 0; w < width; w++)
            {
                ...
                data_to_process = _mm256_xor_si256(input, filter);
                data_to_process = _mm256_popcnt_epi64(data_to_process);
                output_cache_1 = data_to_process + output_cache_4;
                ...
                data_to_process = _mm256_xor_si256(input, filter);
                data_to_process = _mm256_popcnt_epi64(data_to_process);
                output_cache_2 = data_to_process + output_cache_5;
                ...
                filter = filters[f * filter_height * filter_width * depth + i * filter_width + j];
                data_to_process = _mm256_xor_si256(input, filter);
                output_cache_3 = _mm256_popcnt_epi64(data_to_process);
                outputs[output_h * out_width * num_filters + output_w * num_filters + f] += 256 - 2 * (output_cache_3[0] + output_cache_3[1] + output_cache_3[2] + output_cache_3[3]);
