        ...
        for (int h = 0; h < height; h++) {
            for (int w = 0; w < width; w ++) {
                ...
                if (output_h >= 0 && output_h < out_height && output_w >= 0 && output_w < out_width) {
                    ...
                    outputs[output_h * out_width * num_filters + output_w * num_filters + f] += 256 - 2 * (vaddvq_u8(vcntq_u8(output_cache_12.val[0]))+vaddvq_u8(vcntq_u8(output_cache_12.val[1])));
                ...
                if (output_h >= 0 && output_h < out_height && output_w >= 0 && output_w < out_width) {
                    data2 = vld1q_s64_x2((const int64_t *) & filters[(f * filter_height * filter_width + i * filter_width + j)*depth/64]);
                    output_cache_12.val[0] = veorq_s64(input.val[0],data2.val[0]);
                    output_cache_12.val[1] = veorq_s64(input.val[1],data2.val[1]);
                }
